...
export class FirstClassStore {
    ...
    @action async getData(page: number = this.paging.current){
        this.loading = true;
        const Result = await http.getRaw(`web/category_b`,
            {
                PageIndex: page,
                PageSize: 10,
    ...
    @action async editTri() {
        ...
        if (str === "编辑商品分类") {
            const data = {
                ...
                CategoryIcon: this.currItem.CategoryIcon,
            }
            if (this.checkDatas(data)) {
                const res: any = await http.put(undefined, 'web/category_b', data);
                this.loading = false;
                if (res.ResultCode === 0) {
                    this.getData();
                    this.eVisible = false;
            ...
            const data = {
                ...
                CategoryName: this.currItem.CategoryName,
                CategoryIcon: this.currItem.CategoryIcon,
                ParentCategoryId: this.currItem.ParentCategoryId,
            }
            if (this.checkDatas(data)) {
                const res: any = await http.post(undefined, 'web/category_b', data);
                this.loading = false;
                if (res.ResultCode === 0) {
                    this.getData();
                    this.eVisible = false;
    ...
    @action async delUserTri(it: Item) {
        const res = await http.delRaw(`web/category_b?CategoryId=${it.CategoryId}`);
        if (res.ResultCode === 0) {
...
export const firstClassStore = new FirstClassStore();